...
class ObjectDetectionMamager:
    ...
    def __init__(self):
        ...
        self.status = RobotStatus.WAITING # WAITING for the initial robot state
        ...
        self.set_actors_trajectory_srv = rospy.Service(
            'SetActorsTrajectory',
            tiago_msgs.srv.SetActorsTrajectory,
    ...
    def set_actors_trajectory_request(self, request):
        if not self.hparams.fake_sensing:
            ...
            if self.status == RobotStatus.WAITING:
                ...
                return tiago_msgs.srv.SetActorsTrajectoryResponse(True)
            else:
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.hparams.fake_sensing and self.status == RobotStatus.READY:
...
def main():
    rospy.init_node('tiago_object_detection', log_level=rospy.INFO)
    rospy.loginfo('TIAGo object detection module [OK]')

    object_detection_manager = ObjectDetectionMamager()
    object_detection_manager.run()